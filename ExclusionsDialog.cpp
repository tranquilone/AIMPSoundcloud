...
BOOL CALLBACK ExclusionsDialog::DlgProc(HWND hwnd, UINT Msg, WPARAM wParam, LPARAM lParam) {
    switch (Msg) {
                ...
                switch (LPNMHDR(lParam)->code) {
                    case NM_RCLICK: {
                        ...
                        if (i != -1) {
                            ...
                            AppendMenu(popup, MF_STRING, 0x57d001, Plugin::instance()->Lang(L"SoundCloud.Exclusions\\Remove").c_str());
                            AppendMenu(popup, MF_STRING | MF_POPUP, (UINT_PTR)playlistsMenu, Plugin::instance()->Lang(L"SoundCloud.Exclusions\\RemoveAndAddToPlaylist").c_str());
                            AppendMenu(popup, MF_SEPARATOR, 0, NULL);
                            AppendMenu(popup, MF_STRING, 0x57d003, Plugin::instance()->Lang(L"SoundCloud.Menu\\OpenInBrowser").c_str());
                                    ...
                                    switch (result) {
                                        ...
                                        default:
                                            if (auto pl = plMap[result]) {
                                                ...
                                                state->CleanURL = L"http://api.soundcloud.com/tracks/" + std::to_wstring(ti->Id);
                                                SoundCloudAPI::LoadFromUrl(state->CleanURL, pl, state);
        ...
        case WM_INITDIALOG: {
            ...
            SetWindowText (hwnd,                         Plugin::instance()->Lang(L"SoundCloud.Exclusions\\Title").c_str());
            SetDlgItemText(hwnd, IDOK,                   Plugin::instance()->Lang(L"SoundCloud.Exclusions\\OK").c_str());
            SetDlgItemText(hwnd, IDC_EXCLUSIONSGROUPBOX, Plugin::instance()->Lang(L"SoundCloud.Exclusions\\Header").c_str());
            ...
            static auto h1 = Plugin::instance()->Lang(L"SoundCloud.Exclusions\\ListHeader", 0);
            static auto h2 = Plugin::instance()->Lang(L"SoundCloud.Exclusions\\ListHeader", 1);
